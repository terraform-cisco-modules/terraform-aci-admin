...
resource "aci_global_security" "security" {
  ...
  block_duration             = lookup(each.value.lockout_user, "lockout_duration", local.SECURITY.lockout_user.lockout_duration)
  ...
  enable_login_block         = lookup(each.value.lockout_user, "enable_lockout", local.SECURITY.lockout_user.enable_lockout)
  ...
  max_failed_attempts        = lookup(each.value.lockout_user, "max_failed_attempts", local.SECURITY.lockout_user.max_failed_attempts)
  max_failed_attempts_window = lookup(each.value.lockout_user, "max_failed_attempts_window", local.SECURITY.lockout_user.max_failed_attempts_window)
